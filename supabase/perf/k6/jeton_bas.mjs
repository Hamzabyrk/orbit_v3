...
const out = users.map(u => {
  ...
  const payload = b64({ aud: "authenticated", role: "authenticated", sub: u.id, email: u.email, iss: env.API_URL + "/auth/v1", iat: now, exp: now + 3 * 3600, session_id: "00000000-0000-0000-0000-000000000000", is_anonymous: false });
  const sig = createHmac("sha256", env.JWT_SECRET).update(`${header}.${payload}`).digest("base64url");
  return { role: u.role, orbitRole: u.orbitRole, org: u.org, membership: u.membership, token: `${header}.${payload}.${sig}` };
});
