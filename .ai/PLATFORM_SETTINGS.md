...
| ⚠️ ~~Altı~~ **35 indekssiz foreign key (INFO)** — **tetikleyici doldu** | Kayıt "altı" diyordu; 2026-09-05'te ölçüldü ve **35** çıktı. v1.2 on sekiz iş tablosu getirdi, tetikleyicinin ilk yarısı o gün sağlandı ve **kimse bu satıra dönmedi.** Sayının kendisi sessizce eskidi. Orijinal gerekçe hâlâ kısmen geçerli — tablolar boş olduğu için indeks eklemek `unindexed_foreign_keys` uyarısını `unused_index`'e çevirir ve bu **beklenen** sonuçtur; ama artık bilinçli bir iş olarak yapılacaktır, ertelenmiş bir açık olarak değil. | ✅ **Tetiklendi.** Sahibi **v1.2-18**. Aynı dilimde ikinci bir taban ölçüsü de kaydedilecek: **16 × `multiple_permissive_policies` (WARN)** — politikaları rol başına ayırmanın bedeli, satır başına 15 `security definer` yardımcı. Yük testi tablolar boşken anlamsız; bu sayılar gerçek veri geldiğinde karşılaştırılacak tabandır |
