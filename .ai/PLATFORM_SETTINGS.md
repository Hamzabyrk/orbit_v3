...
| `main` merge'ü ikinci kişiye bağlı — **ama dolaylı bir mekanizmayla** | Her merge için ikinci kişi gerekiyor ve bu bugün çalışıyor. Nasıl çalıştığı ise sezgisel değil: `main-koruma` ruleset'inde `required_approving_review_count` **0**'dır; kuralı işleten şey `require_extra_approval_for_unattributed_changes` ayarıdır. Bu ayar, kişiye atfedilmemiş değişiklik taşıyan PR'larda gereken onay sayısını **bir artırır** ve commit'lerimiz `Co-Authored-By: Claude` taşıdığı için her PR o sınıfa girer. Sonuç doğru, dayanağı kırılgan. Ayrıntı aşağıda. | **Ekip üyelerinden biri uzun süre erişilemez olursa** — ve ayrıca aşağıdaki kırılganlık gerçekleşirse                                                        |
