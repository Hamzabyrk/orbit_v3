...
| v1.3 (kalan)  | Ekranların canlı sorguya bağlanması, hesaplar arası geçiş ve kişi kaydı — mock temizliği E5'te bitti, **kalanın dilimleri §4.6'da**. Realtime **v1.3-05** olarak eklendi (2026-09-05). **2026-09-07: dilimler yeniden kesildi** — dört bakım dilimi (v1.3-07…10) v1.3-01 öncesine alındı ve v1.3-06 açıldı                     | ⬜    |
...
- [ ] **Ortam değişkeni doğrulaması.** `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` yoksa production build'i **düşer**. Bugün `placeholder.supabase.co`'ya düşülüyor ve yanlış yapılandırılmış bir dağıtım yalnızca `console.warn` bırakarak ölü bir uygulama yayınlıyor — hata görünmez, çünkü hata yoktur.
...
| Dilim       | Kapsam                                                                                                                                 | Dayandığı varsayımlar                                                                                                                                                                                                                                                                      |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **v1.3-00** | 🟠 **Mimari kararlar** — veri deseni, servis yerleşimi, sayfalama sözleşmesi, gün gösterimi, env doğrulaması, `PROJECT_STATE` §5 ağacı | v1.2-22 · **v1.3-01'den önce koşar.** Kapsamı §4'te madde madde yazılı; kaynağı §4.9'un yedi bulgusu — **yedisi de bu dilime düştü**, hepsi "ekran yazmadan önce kararı verilmemiş" cinsinden                                                                                              |
| **v1.3-07** | **#213** — kimlik çözümü sürerken yapılan çıkış geri alınıyor                                                                          | Bağımsız · Canlı hata ve **her ekranın altındaki katmanda**; v1.3 boyunca her dilim bu katmanın üstüne yazacak. Düzeltmesi küçük, sabitleyen test şart                                                                                                                                     |
| **v1.3-08** | **#212** — belgelenmemiş `wouter` yamasının kaldırılması                                                                               | Bağımsız · ⏰ **Zamanlı:** yama `wouter@3.7.1`'e sabit, aralık `^3.3.5` ve mevcut sürüm 3.11.0 — bir **minor**. Bir sonraki aylık gruplu npm PR'ı onu yükseltince yama tutmayacak ve `pnpm install` kırılacak                                                                              |
| **v1.3-09** | **#214 · #216** — SHA yorumları ve CI yüzeyindeki dört ayrışma                                                                         | Bağımsız · Bundan sonraki her PR'ın CI'ına güvenilmesi buna bağlı: bugün yorumların beşi de yanlış ve yığılı PR'lar CodeQL taraması almıyor · **K-21** bu dilimden çıktı                                                                                                                   |
| **v1.3-10** | **#215** — Edge Function bağımlılıklarının kapsama alınması                                                                            | Bağımsız · Sistemin tek ayrıcalıklı tarafı, istemciden 70 minor geride ve hiçbir kapının kapsamında değil · Yükseltme sonrası `create_member_chain.sh` zincir kanıtı koşar                                                                                                                 |
| **v1.3-01** | `educationData.ts` yerine gerçek servisler                                                                                             | v1.2 tamamı · **v1.3-00** · `components/` Supabase istemcisini import edemez (taşınabilirlik sınırı, ESLint zorluyor) · ⚠️ Kural bugün `hooks/`'u kapsamıyor; v1.3-00 onu genişletmeden bu dilim kuralın dışına yazabilir                                                                  |
| **v1.3-02** | Yükleme ve hata durumları · `<StrictMode>` · **#196**                                                                                  | v1.3-01 · `skeleton.tsx` var ama hiçbir eğitim ekranı kullanmıyor · Üçü aynı sınıfa bakıyor: efekt saflığı. #196'nın kontrol noktası zaten burasıydı                                                                                                                                       |
| **v1.3-03** | ✅ Giriş ekranının uydurma verisi + demo verisinin paketten çıkarılması (**#144**)                                                     | Bağımsızdı, kapandı                                                                                                                                                                                                                                                                        |
| **v1.3-06** | **Mevcut iki listenin sözleşmeye geçirilmesi** — `audit` ve `platform`                                                                 | **v1.3-00** (sözleşme orada yazılır) · Yeni ekran değil, **yayında olan** iki ekranın sessiz kesmesinin kapatılması · Denetim kaydı bir uyum yüzeyidir; "en yeni 50" bir uyum cevabı değildir                                                                                              |
| **v1.3-04** | Hesaplar arası geçiş düğmesi ve kişi kaydı                                                                                             | Çoklu hesap kararı (`DECISION_LOG` 2026-08-25) · ⚠️ **zemin notu 2026-08-29**: oturum artık `sessionStorage`'da, kararın iki paragrafı bu dünyada yazılmadı · **v1.3-07 önce biter**: iki oturumun birden saklandığı bir dünyada bayat kimlik yazımı çok daha kolay yanlış hesabı gösterir |
...
| **v1.4-07** | Üye satır işlemleri: rol değiştirme, kurumdan çıkarma                                                      | E6'dan kalan · `CredentialsPanel` `components/credentials/` altında ortak · ⚠️ **v1.2-15 zemin kaydı:** ayakta duran ders ataması varken rol değiştirme artık veritabanında **reddediliyor** (`ORB03`). Ekran bunu ele almalı: önce atamaları arşivlet, sonra rolü değiştir — yoksa yönetici açıklanamayan bir hatayla karşılaşır · ⚠️ **v1.2-17 zemin kaydı:** tekrarlanan istek bugün arayüzde bir **hata** olarak görünüyor, oysa iş **başarılı** olmuştur. Mesaj bunu söylüyor ama kırmızı kutu yanıltıcı; kimlik bilgisi paneli zaten bu dilimde ele alınıyor, ayrı bir "zaten işlendi" görünümü oraya ait |
...
| **v1.4-13** | Gün Planı görev ve takvim yönetimi                                                                         | v1.2-09 · ⚠️ `dayPlanTasksByRole` **role göre** kurulu; kullanıcı bazına taşınması v1.3-01'in işi (K-11) · ⚠️ **Takvim bileşeni depoda yok (2026-09-07).** shadcn takvimi kullanılmadığı ve v10 kırıcı değişikliğinde tsc kırdığı için kaldırıldı (#210); gerektiğinde `npx shadcn add calendar` ile gelir ve o gün zaten v10 uyumlu kurulur. Silme sessiz olmasın diye buraya yazıldı                                                                                                                                                                                                                          |
