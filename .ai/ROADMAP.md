...
| v1.2          | İş tabloları + tenant/rol RLS matrisi — **dilimleri §4.6'da**. On iki dilim, on sekiz iş tablosu, 98 RLS politikası, 382 pgTAP iddiası. ⚠️ **2026-09-05'te tamamlandı ilan edildi, aynı gün geri alındı:** kapsam turu iki politika bütünlüğü açığı buldu ve biri **testle doğru davranış ilan edilmişti**. Release gate yeniden ölçülmeden ✅ olamaz — **v1.2-22**. Yama dilimleri v1.2-13…22, ayrıntı **§4.8** | ⚠️    |
...
| ----------- | ---------------------------------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **v1.2-13** | 🔴 **Dağıtım kapsamı ölçülür** — `create-member` → `config.toml` + CI kapısı             | 3        | Merge yalnızca `config.toml`'daki fonksiyonları deploy ediyor (2026-09-05'te ölçüldü) · Kapı **K-19**'un karşılığıdır: liste karşılaştırması testle sabitlenir |
| **v1.2-14** | 🔴 **Politika bütünlüğü** — yoklama öğrenci↔sınıf, sınav öğrenci↔sınav                 | 1, 2     | v1.2-04 ve v1.2-05 production'da · ⛔ **Mevcut bir test açığı doğru davranış ilan ediyor**, düzeltilince kırmızıya döner ve bu bir regresyon değildir          |
| **v1.2-15** | 🟠 **Rol korkuluğu** — üç üyelik sütununda `student`/`parent` dışlanır                   | 5, 6, 7  | v1.2-02 ve v1.2-07 production'da · Kısıt "teacher olmalı" **değil**, "student/parent olmamalı" — admin ders verebilir (v1.2-02 kararı)                         |
| **v1.2-16** | 🟠 **Edge Function dürüstlüğü ve telafisi**                                              | 8, 9, 17 | `create-member` `audit_written`'ı **sabit** yazıyor, `reset-member-password` **hesaplıyor** · Kilit hatası görünür ama çaresi yok                              |
| **v1.2-17** | 🟠 **Hız sınırı + idempotency** — beş fonksiyona ortak katman                            | 10       | Bugün hiçbir fonksiyonda yok (ölçüldü: sıfır eşleşme) · **Bu turun en büyük dilimi**; v1.5-04 "denetim"den "tasarım"a döner                                    |
| **v1.2-18** | 🟠 **İndeksler + §5 tazelenmesi** — 35 FK, taban ölçüsü kaydı                            | 11, 14   | §5'in tetikleyicisi doldu · İndeks eklemek `unindexed_foreign_keys`'i `unused_index`'e çevirir ve bu **beklenen** sonuçtur                                     |
| **v1.2-19** | 🟡 **Küçük bütünlük** — `subject_id` sınıfa bağlanır, arşiv tutarsızlığı karara bağlanır | 15, 16   | `owns_student_record` `archived_at`'a **bakmıyor**, `guards_student` **bakıyor** — hangisinin doğru olduğu karara bağlanmamış                                  |
| **v1.2-20** | 🟡 **CI sertleştirme** — SHA pinning, CodeQL, bağımlılık ve lisans                       | 19, 20   | Action'lar bugün sürüm etiketiyle (`@v4`, `@v1`) · Repo **public**, tedarik zinciri yüzeyi gerçek                                                              |
| **v1.2-21** | 🟡 **Edge Function entegrasyon testleri** — Auth→EF→SQL zinciri                          | 21       | v1.2-13 · pgTAP SQL yetkisini iyi sınıyor, **zinciri sınamıyor** · Staging **yok** (2026-09-05 kararı), testler bu kısıtla tasarlanır                          |
| **v1.2-22** | ✅ **Release gate yeniden ölçülür**                                                      | —        | v1.2-13…21 tamamı · Gate'in **metni değişmez**, doğrulaması yenilenir                                                                                          |

> ⛔ **v1.2-14 bir testi kırmızıya çevirecek ve bu istenen sonuçtur.** `attendance.test.sql`'deki `lives_ok`, 12-A öğretmeninin 12-B öğrencisine kayıt yazmasını **"a teacher can record attendance in a session of their own class"** etiketiyle onaylıyor. Etiket oturum yarısını anlatıyor, öğrencinin yabancı olduğunu hiç söylemiyor. **Test açığı yakalamıyor, çiviliyor.**
...
| Dilim       | Kapsam                                                                                                 | Dayandığı varsayımlar                                                                                                                                                                                                                               |
| ----------- | ------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **v1.3-00** | 🟠 **Mimari kararlar** — veri deseni, servis yerleşimi, sayfalama sözleşmesi, `PROJECT_STATE` §5 ağacı | v1.2-22 · **v1.3-01'den önce koşar.** Bugün iki çelişen desen var: React Query kurulu ve mount edilmiş ama **sıfır** `useQuery`; `AuditLogPage` elle `useEffect`. Servis yerleşimi de çelişiyor: `platform/` servis+bileşen, `audit/` yalnız servis |
