...
>
> **Açık bulgular `gh issue list`'tedir.** Sayı buraya yazılmıyor: denetimden bir gün sonra eskidi (**K-06**). Sıradaki dilim **v1.2-10** (kapsam çözümleyicisi). Bu, v1.2'nin **ilk istemci dilimi**: dokuz dilimdir veritabanında kurulan kapsam ekrana bağlanıyor ve `scopeFilters.ts`'in üretimde boş küme dönmesi sona eriyor. Üç K-11 kaydı orada karşılığını bulacak. Sistemin bugünkü durumu: `PROJECT_STATE.md` **§6.1**.
...
| v1.2          | İş tabloları + tenant/rol RLS matrisi — **dilimleri §4.6'da**. v1.2-01…09 kapandı; on sekiz iş tablosu ve 97 RLS politikası ayakta                                                                                                                                                            | 🟡    |
...
> ⚠️ **v1.2-07 bir K-11 kaydı bıraktı.** Veritabanı haftanın **yedi** gününü kabul ediyor; istemcideki `WeekDay` tipi yalnızca **beş** (Pazartesi–Cuma). Ekran bu tabloya bağlandığında (v1.2-10) ya tip yediye genişletilmeli ya da hafta sonu dersleri **açıkça** kapsam dışı ilan edilmeli. Sessizce dar kalırsa sonuç, kaydedilmiş ama hiçbir ekranda görünmeyen ders satırlarıdır — kullanıcının veri kaybı sandığı, aslında görüntülenmeyen kayıtlar.
